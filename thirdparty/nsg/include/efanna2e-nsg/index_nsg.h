...
#include "efanna2e-nsg/neighbor.h"
...
class IndexNSG : public Index {
 ...
 public:
  ...
  void SetInitialGraph(CompactGraph graph) {final_graph_ = graph;}
  void BuildLoadedGraph(size_t n, const float *data, const Parameters &parameters);
  virtual void Search(
      const float *query,
      const float *x,
      size_t k,
