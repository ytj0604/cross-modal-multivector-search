...
int main(int argc, char **argv) {
  ...
  try {
    ...
    // desc.add_options()("gt_path",
    // po::value<std::string>(&gt_file)->required(), "Groundtruth file in bin
    // format"); desc.add_options()("query_data_path",
    ...
    //                    "Path prefix for saving bipartite index file
    //                    components");
    desc.add_options()(
  ...
  // efanna2e::load_gt_data_with_dist<uint32_t, float>(gt_file.c_str(), gt_pts,
  // gt_dim, gt_ids, gt_dists);
  ...
  efanna2e::IndexBipartite index(q_dim, base_num + sq_num, dist_metric,
                                 nullptr);

  index.LoadSearchNeededData(base_data_file.c_str(),
                             sampled_query_data_file.c_str());
  ...
  // std::vector<std::vector<float>> res_dists(q_pts, std::vector<float>(k,
  // 0.0));
  uint32_t *projection_cmps_vec =
      (uint32_t *)aligned_alloc(4, sizeof(uint32_t) * q_pts);
  memset(projection_cmps_vec, 0, sizeof(uint32_t) * q_pts);
  uint32_t *hops_vec = (uint32_t *)aligned_alloc(4, sizeof(uint32_t) * q_pts);
  float *projection_latency_vec =
      (float *)aligned_alloc(4, sizeof(float) * q_pts);
  ...
  if (!evaluation_save_path.empty()) {
    ...
    if (!evaluation_out.is_open()) {
      std::cerr << "Error opening evaluation file: " << evaluation_save_path
                << std::endl;
      return -1;
    }
  ...
  std::string evaluation_file_path = evaluation_save_prefix + "_" +
                                     std::to_string(max_pq_size_budget) +
                                     ".tsv";
...
#pragma omp parallel for schedule(dynamic, 1)
  for (size_t i = 0; i < q_pts; ++i) {
    ...
    for (int32_t j = 0; j < query_multivector_size; j++) {
      queries[j] =
          aligned_query_data + i * q_dim * query_multivector_size + j * q_dim;
    }
    ...
    auto ret_val = index.SearchMultivectorOnRoarGraph(
        queries, k, i, parameters, indices, res_dists,
        enable_adaptive_expansion);
    ...
    auto rerank_diff = std::chrono::duration_cast<std::chrono::microseconds>(
                           query_end - rerank_start)
                           .count();
    auto rerank_time_seconds = static_cast<double>(rerank_diff) /
                               1'000'000;  // Convert to seconds with precision
    auto query_diff = std::chrono::duration_cast<std::chrono::microseconds>(
                          query_end - query_start)
                          .count();
    double query_time_seconds = static_cast<double>(query_diff) /
                                1'000'000;  // Convert to seconds with precision
...
#pragma omp critical
  ...
  evaluation_out << recall << "\t" << QPS << "\t"
                 << total_rerank_time /
                        (total_rerank_time + total_graph_search_time)
                 << "\n";
  if (evaluation_out.is_open()) {
